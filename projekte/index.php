...
$membersQuery = mysqli_query($con, "SELECT * FROM projekts");
$members = array();
...
<nav class="nav custom-nav">
  ...
  <a class="nav-link active" href="">Projekte</a>
  <a class="nav-link" href="/logout.php">Logout</a>
    ...
    <table>
      ...
      <div class="box" style="border-radius: 5px;">
          ...
          <li class="nav-item inline">
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title" id="exampleModalLongTitle">Löschen des Projektes</h5>
                      <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                      </button>
                    </div>
                    <div class="modal-body">
                      Bist du dir sicher das du das projekt löschen möchtest?
                    </div>
                    <div class="modal-footer">
                      <button type="button" class="btn btn-secondary" data-dismiss="modal">Abrechen</button>
                      <button type="button" class="btn btn-primary" onclick="DeleteProject('<?=$member['name']?>')">Löschen</button>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <li class="nav-item inline dropdown">
              <a class="nav-link" data-toggle="dropdown">
                <i onclick="EditProject('<?=$member['name']?>')" class="material-icons md-18">
        ...
        <div class="box-body" style="font-family: 'Poppins', serif; font-size: 20px;">
          <p class="m-0">Personen: <?=$member['persons']?></p>
        </div>
      </div>
    </div>
            <?php
    ...
    <script>
      ...
      function EditProject(str) {
        ...
        window.location.href = "project.php?q="+str;
      }
    </script>
